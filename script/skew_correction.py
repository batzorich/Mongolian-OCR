...
def skew_correction(path_to_file):
    ...
    parent_dir, filename = os.path.split(path_to_file)
    new_parent_dir = os.path.join(os.path.dirname(parent_dir), 'corrected_images')
    
    if not os.path.exists(new_parent_dir):
        os.makedirs(new_parent_dir)
    
    new_file_path = os.path.join(new_parent_dir, filename)
    cv2.imwrite(new_file_path, rotate_img)
